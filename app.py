...
### License: MIT
import os, re, http_client
#import filesystem

EMF_USER = "emf"
USER_NAME_SEPARATOR = "~"
ATTRIBUTE_MATCHER = re.compile("^\s*###\s*([^:]*?)\s*:\s*(.*)\s*$") # Yeah, regex!
CATEGORY_ALL = "all"
...
class App:
	...
	def loadable(self):
		...
		return os.path.isfile(self.main_path) and os.stat(self.main_path)[6] > 0
...
def get_local_apps(category=CATEGORY_ALL):
	...
	apps = [App(folder_name) for folder_name in os.listdir("apps") if os.path.isdir("apps/" + folder_name)]
